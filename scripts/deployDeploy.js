...
async function main() {
  ...
  const containerDeployFactory = await ethers.getContractFactory(
    "ContainerDeploy"
  );
  ...
  console.log(
    `ContainerDeploy deployed on ${network.name}, contract address is ${containerDeploy.address} `
  );
  ...
  console.log(`transfer ContainerDeploy with ${fundAmount / 10 ** 18} Link`);
}
