...
async function main() {
  ...
  console.log(
    `ZtControl deployed on ${network.name}, contract address is ${ztControl.address} `
  );
  ...
  console.log(
    `transfer ZtControl with ${fundAmount / 1000000000000000000} Link`
  );

  const set = await ztControl.setSites(
    networkConfig[chainId]["sitename"],
    networkConfig[chainId]["ztMeshJob"],
    networkConfig[chainId]["oracle"],
    networkConfig[chainId]["sitehost"]
  );

  console.log(
    "Contract ",
    ztControl.address,
    "setSites success, hash: ",
    set.hash
  );
}
