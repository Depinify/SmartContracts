...
async function main() {
  ...
  const containerClusterFactory = await ethers.getContractFactory(
    "ContainerCluster"
  );
  const containerCluster = await containerClusterFactory.deploy(
    linkTokenAddress
  );
  ...
  console.log(
    `ContainerCluster deployed on ${network.name}, contract address is ${containerCluster.address} `
  );
  ...
  console.log(
    `transfer ContainerCluster with ${fundAmount / 1000000000000000000} Link`
  );
}
