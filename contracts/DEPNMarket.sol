...
contract DEPNMarket is ERC721URIStorage {
    ...
    constructor() ERC721("DEPNMarket NFT", "DEPNFT") {
    ...
    function createUserNft(
        ...
        for (uint i = 0; i < _userSoulIds[msg.sender].length; i++) {
            require(
                _idToUserSoul[_userSoulIds[msg.sender][i]].daoId == daoId,
                "Is Created"
            );
        }
    ...
    function createMarketApi(
        ...
        require(
            _idToDao[daoId].daoOwner == msg.sender,
            "No permission to create a market"
        );
        uint256 latestMarketId = _marketIdCounter.current();
        _marketIdCounter.increment();

        _idToApiMarket[latestMarketId] = ApiMarket(
            latestMarketId,
            daoId,
            apiName,
            apiMethod,
    ...
    function getMarketApiByDao(
        uint256 daoId
    ) external view returns (ApiMarket[] memory) {
        uint256 marketId;
    ...
    function checkToCreateMarket(uint256 daoId) external view returns (bool) {
