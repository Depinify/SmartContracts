...
task("set-sites", "Call ZtControl Contract to configure site data")
  .addOptionalParam(
    "contract",
    "The address of the ZtControl contract that you want to call"
  )
  ...
  .setAction(async taskArgs => {
    ...
    const contractAddr =
      taskArgs.contract || networkConfig[networkId]["ZtControl"];
    ...
    const set = await ztControlContract.setSites(name, jobid, oracle, host);
    console.log("Contract ", contractAddr, "Transaction Hash: ", set.hash);

    await set.wait();

    const result = await ztControlContract._sites(name);
    console.log("Contract ", contractAddr, " Configure site data: ", result);
  });
module.exports = {};
