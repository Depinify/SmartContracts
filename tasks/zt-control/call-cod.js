...
task("call-cod", "Calls an codControl Contract to request external data")
  .addParam(
    "contract",
    "The address of the codControl contract that you want to call"
  )
  ...
  .setAction(async taskArgs => {
    ...
    const codControlContract = await ethers.getContractAt(
      "CodControl",
      contractAddr
    );

    const result = await codControlContract.CallCOD(
      formatBytes32String(requestid),
      sitename,
      url,
      httpmethod,
      requestdata
